...
<div class="container">
  ...
  <!-- 更新・削除フォーム -->
  <form action="t002_make.php" method="post" class="form-horizontal">
        ...
        <tbody class="datalist">
          ...
          if(!empty($visit_list)){
            foreach($visit_list as $visit){
              $hosp_name = get_hosp_name($visit["hosp_code"]);
              ?>
              <tr class="record">
                <input type="hidden" name="visit_id[]" value="<?php echo $visit["visit_id"];?>">
                <td>
                  <input type="checkbox" class="form-control" name="del-<?php echo $visit["visit_id"];?>">
                </td>
                <td>
                  <input type="text" class="form-control" name="hosp-name[]" value="<?php echo htmlspecialchars($hosp_name, ENT_QUOTES, "UTF-8");?>" readonly>
                </td>
                <td>
                  <input type="date" class="form-control" name="plan_date[]" value="<?php echo htmlspecialchars($visit["plan_date"], ENT_QUOTES, "UTF-8");?>" readonly>
                </td>
                <td>
                  <input type="date" class="form-control" name="record_date[]" value="<?php echo htmlspecialchars($visit["record_date"], ENT_QUOTES, "UTF-8");?>">
                </td>
                <td>
                  <textarea rows="3" class="form-control" name="note[]"><?php echo htmlspecialchars($visit["note"], ENT_QUOTES, "UTF-8");?></textarea>
                </td>
              </tr>
              <?php
            }
